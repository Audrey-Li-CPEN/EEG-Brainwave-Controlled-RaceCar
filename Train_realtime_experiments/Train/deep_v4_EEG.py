...
class DeepConvNet(nn.Module):
    def __init__(self, num_channels=14, num_classes=4):
        ...
        # Additional convolution to reduce feature size
...
# Main Script
def preprocess_data(file_path, label_mapping, fs=128, lowcut=8.0, highcut=50.0, window_size=140, overlap=122):
    # Read data
...
def preprocess_multiple_files(file_paths, label_mapping):
    ...
    for file_path in file_paths:
        ...
        windows, labels, scaler = preprocess_data(
            file_path, 
            label_mapping,
            window_size=140,
            overlap=130
        ...
        if first_file:
            ...
            joblib.dump(scaler, 'scaler_v4_11.pkl')
            first_file = False
...
# Training
def train_model(model, train_loader, val_loader, device, save_path, epochs=500):
...
if __name__ == "__main__":
    ...
    save_path = r"D:\Train\model_v4_11.pth"

    # Define label mapping
    label_mapping = {'left': 0, 'rest': 1, 'right': 2, 'stop': 3}

    # Preprocess data with new window size and overlap
